...
public class QuestionLineFileStrategyServiceImpl extends DefaultFileStrategyServiceImpl {
    ...
    public NBBase uploadFile(NBBase nbBase, String propertyName, String fileName, InputStream inputStream) throws ClientException {
        try {
            QuestionLine questionLine = (QuestionLine) baseService.findEntity(nbBase, false);
            validationUploadAuthority(questionLine);
     ...
     */
    private void validationUploadAuthority(QuestionLine questionLine) {
        if (!StringUtils.isNullOrEmpty(questionLine.getReserved7())) {
            if (!questionLine.getReserved7().equals(ThreadLocalContext.getUsername())){
            ...
            questionLine.setReserved7(ThreadLocalContext.getUsername());
            questionLine.setReserved8(user.getDescription());
        }
    }
}
